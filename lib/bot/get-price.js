...
async function getPriceForFlight ({ from, to, date, number, isPointsBooking, passengerCount, alertType, nonStopOnly }, browser, lock) {
...
async function getFlights ({ from, to, departDate, returnDate, isPointsBooking, browser, passengerCount, lock }) {
  ...
  try {
    if (lock) {
      ...
      html = await getPage(from, to, departDate, returnDate, isPointsBooking, passengerCount, browser);
      await lock.signal();
...
function createUrl (from, to, departDate, returnDate, isPointsBooking, passengerCount) {
...
async function getPage (from, to, departDate, returnDate, isPointsBooking, passengerCount, browser) {
  ...
  try {
    const url = createUrl(from, to, departDate, returnDate, isPointsBooking, passengerCount);
    ...
    try {
      // TODO where is the default wait time defined?
      await page.waitForSelector('.price-matrix--details-titles');
      await page.waitForSelector('.flight-stops-badge');
