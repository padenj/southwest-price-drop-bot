  ...
  try {
	  ...
	  let countButtonPushes = 0;
          while (refreshPage) {
             ...
             try {
                 await page.waitForSelector('.price-matrix--details-titles',{ timeout: 20000 });
                 console.log('Found departing flights!!');
                 refreshPage = false;
             } catch (err) {
                 console.log('find price error: ', err)
                 await page.screenshot({ path: 'failed2.png', fullPage: true })
          ...
          console.error('Unable to get flights', err);
